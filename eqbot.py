...
class EqBot(LinearBot):
    ...
    def _setup(self, ltp_quote):
        ...
        ltp = ltp_quote['ltp']
        levels = ltp#gann(ltp)
        self.buy = ltp #levels[DEFAULTS['buy']]
        self.target = ltp + 10 #levels[DEFAULTS['target']]
        self.stoploss = ltp -10 #gann(ltp, 'down')[DEFAULTS['stoploss']]
        self.prev_ltp = ltp
        self._print_levels()
        self.state.append('setup complete')

    def _sellsetup(self, ltp_quote):
        self.instrument = ltp_quote['instrument']
        ltp = ltp_quote['ltp']
        levels = ltp#gann(ltp)
        self.buy = ltp #levels[DEFAULTS['buy']]
        ...
        self.prev_ltp = ltp
        # levels = gann(ltp)
        # self.sell = levels[DEFAULTS['sell']]
        # self.target = levels[DEFAULTS['target']]
        # self.stoploss = gann(ltp, 'up')[DEFAULTS['stoploss']]
        # self.prev_ltp = ltp
        self._print_levels()
        self.state.append('setup complete')
